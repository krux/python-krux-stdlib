                ...
                if len(outputs[1]):
                    ...
                    for s in outputs[1].splitlines():
                        ...
                        for r in filters:
                            ...
                            if r.search(s):
                                log.debug('line "%s" matches "%s"' %
                                          (s, r.pattern))
                                ignore = True

                        # not filtered - store the filtered output in the
                        # object, so caller can inspect them
...
class IO(object):
    ...
    def run_cmd(self, raise_exception=False, *args, **kwargs):
        ...
        :keyword filters: list of output filters to apply to stdout/stderr
        before capturing or logging it. Filters can be strings or regular
        expressions.
